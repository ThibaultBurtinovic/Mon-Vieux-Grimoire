    ...
    Book.findOne({ _id: req.params.id })
        .then((book) => {
           ...
           // Ajouter la nouvelle note au tableau des notes
            ...
            // Calculer la note moyenne des notes
            const totalRatings = book.ratings.length;
            const totalRatingSum = book.ratings.reduce((sum, r) => sum + r.grade, 0);
            book.averageRating = totalRatings > 0 ? totalRatingSum / totalRatings : 0;
...
exports.getBestRating = (req, res, next) => {
    // Utiliser la méthode find() pour récupérer les trois livres avec la meilleure note moyenne
    Book.find()
        .sort({ averageRating: -1 }) // Tri décroissant en fonction de la note moyenne
        .limit(3) // Limiter à trois livres
        .then((bestRatedBooks) => {
